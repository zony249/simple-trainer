...
from transformers.tokenization_utils_base import PaddingStrategy, BatchEncoding
...
# full_path_to_add = os.path.join(script_dir, "data")
...
#         sources = []
#         # Tokenize sources, truncate to max length, add gist tokens, and convert back to string.
...
class DataCollatorForAlpacaCLM:
    ...
    def __call__(self, batch, 
        ...
        """
        Right now, tokenizer and use_chat_template do nothing
        """
        if any("human" in instance["split"] for instance in batch):
            # Use the human max lengths.
